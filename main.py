...
# def select_path():
#     # Function to handle path selection (modify as needed)
#     filedialog.askdirectory()

# def get_user_playlists(access_token):
#     # Placeholder function to simulate playlist fetching
...
# def stop_downloading():
#     # Placeholder function to simulate stopping downloads
...
screen.title('Download Your Spotify Playlist: ')

# Styling
style = ttk.Style(screen)
style.theme_use('clam')

# Configure custom styles for Spotify-like appearance
style.configure("TFrame", background="black")
style.configure("TLabel", background="black", foreground="white", font=("Arial", 14, "bold"))
style.configure("TButton", background="green", foreground="white", font=("Arial", 12, "bold"), padding=5)
style.configure("", background="green", foreground="white", font=("Arial", 12, "bold"), padding=5)
style.map("TButton", background=[("active", "#1DB954")])  # Spotify green for active buttons
...
logo_row.pack(fill='x', pady=20)
logo_label = Label(logo_row, background="black")
logo_label.pack(side='left', padx=5)

logo_image = Image.open("images/spotify_logo.png")  # Ensure the file is in the same directory
logo_image = logo_image.resize((300, 100), Image.LANCZOS)  # Resize the image using LANCZOS
logo_photo = ImageTk.PhotoImage(logo_image)
logo_label.configure(image=logo_photo)
# Path selection (left-aligned)
path_row = ttk.Frame(frame)
path_row.pack(fill='x', pady=10)
path_label = ttk.Label(path_row, text="Select Download Path:")
path_label.pack(side='left', padx=5)
select_path_button = ttk.Button(path_row, text="Browse", command=select_path)
select_path_button.pack(side='left', padx=5)

# Playlist selection (left-aligned)
playlist_row = ttk.Frame(frame)
playlist_row.pack(fill='x', pady=10)
playlist_label = ttk.Label(playlist_row, text="Select Loading Playlist:")
...
selected_playlist = StringVar()
playlist_dropdown = ttk.OptionMenu(playlist_row, selected_playlist, "Loading playlists...")
playlist_dropdown.pack(side='left', padx=5)

# Placeholder for fetching playlists (call after GUI setup)
get_user_playlists(access_token)

# Download button (left-aligned)
download_row = ttk.Frame(frame)
download_row.pack(fill='x', pady=10)
download_label = ttk.Label(download_row, text="Start downloading the playlist:")
download_label.pack(side='left', padx=5)
download_button = ttk.Button(download_row, text="Download", command=start_download)
download_button.pack(side='left', padx=5)

# Stop button (left-aligned)
stop_row = ttk.Frame(frame)
stop_row.pack(fill='x', pady=10)
stop_label = ttk.Label(stop_row, text="Stop the current download:")
stop_label.pack(side='left', padx=5)
stop_button = ttk.Button(stop_row, text="Stop Downloading", command=stop_downloading)
